    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                default:
                    ModelState.AddModelError("", "اطلاعات کاربری نادرست است");
                    return View(model);
            }
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                bool resultGetUserType = false;
                var defaultUserType = DmlObj.GetDefaultUserType(out resultGetUserType);
                if (!resultGetUserType) return View("Error");

                bool resultGetDefaultImage = false;
                var DefaultImage = DmlObj.GetDefaultImage(out resultGetDefaultImage);
                if (!resultGetDefaultImage) return View("Error");

                var amUser = new DAL.AMUser
                {
                ...
                bool resultAddAmUser = false;
                var AddedUserObj = DmlObj.AddAmUser(amUser, out resultAddAmUser);
                if (!resultAddAmUser) return View("Error");

                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, AMUserId = AddedUserObj.Id };
